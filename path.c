 ...
 * Copyright 2026 Katayama Hirofumi MZ
 *
...
HRESULT WINAPI PathCchAddBackslash(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size)
{
    return PathCchAddBackslashEx(path, size, NULL, NULL);
}

HRESULT WINAPI PathCchAddBackslashEx(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size, _Outptr_opt_result_buffer_(*pcchRemaining) PWSTR* endptr, _Out_opt_ size_t* remaining)
{
...
HRESULT WINAPI PathCchAddExtension(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size, _In_ PCWSTR extension)
{
...
HRESULT WINAPI PathCchAppend(_Inout_updates_(cchPath) PWSTR path1, _In_ size_t size, _In_opt_ PCWSTR path2)
{
...
HRESULT WINAPI PathCchAppendEx(_Inout_updates_(cchPath) PWSTR path1, _In_ size_t size, _In_opt_ PCWSTR path2, _In_ DWORD flags)
{
    HRESULT hr;
    WCHAR *result;
...
HRESULT WINAPI PathCchCanonicalize(_Out_writes_(cchPathOut) PWSTR out, _In_ size_t size, _In_ PCWSTR in)
{
...
HRESULT WINAPI PathCchCanonicalizeEx(_Out_writes_(cchPathOut) PWSTR out, _In_ size_t size, _In_ PCWSTR in, _In_ DWORD flags)
{
    WCHAR *buffer;
    SIZE_T length;
    HRESULT hr;
...
HRESULT WINAPI PathCchCombine(_Out_writes_(cchPathOut) PWSTR out, _In_ size_t size, _In_opt_ PCWSTR path1, _In_opt_ PCWSTR path2)
{
...
HRESULT WINAPI PathCchCombineEx(_Out_writes_(cchPathOut) PWSTR out, _In_ size_t size, _In_opt_ PCWSTR path1, _In_opt_ PCWSTR path2, _In_ DWORD flags)
{
    HRESULT hr;
    WCHAR *buffer;
    SIZE_T length;
...
HRESULT WINAPI PathCchFindExtension(_In_reads_(cchPath) PCWSTR path, _In_ size_t size, _Outptr_ PCWSTR* extension)
{
...
HRESULT WINAPI PathCchRemoveBackslash(_Inout_updates_(cchPath) PWSTR path, _In_ size_t path_size)
{
    WCHAR *path_end;
    size_t free_size;
...
HRESULT WINAPI PathCchRemoveBackslashEx(_Inout_updates_(cchPath) PWSTR path, _In_ size_t path_size, _Outptr_opt_result_buffer_(*pcchRemaining) PWSTR* path_end, _Out_opt_ size_t* free_size)
{
    const WCHAR *root_end;
    SIZE_T path_length;
...
HRESULT WINAPI PathCchRemoveExtension(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size)
{
...
HRESULT WINAPI PathCchRemoveFileSpec(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size)
{
    WCHAR *last, *root_end;
...
HRESULT WINAPI PathCchRenameExtension(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size, _In_ PCWSTR extension)
{
    HRESULT hr;
...
HRESULT WINAPI PathCchStripPrefix(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size)
{
    TRACE("%s %Iu\n", wine_dbgstr_w(path), size);
...
HRESULT WINAPI PathCchStripToRoot(_Inout_updates_(cchPath) PWSTR path, _In_ size_t size)
{
    const WCHAR *root_end;
    WCHAR *segment_end;
